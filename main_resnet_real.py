...
if __name__=='__main__':
    ...
    #Load model
    mfile='/results/multi_rank_1_7_sim_Res3d_Model_Tr6000_epoch30_lr2.00e-03.pkl'
    ...
    minloss=np.inf
    #Logs
    log=open('/results/%s/%s_Res3d_Log_Tr%s_epoch%s_lr%.2e.txt'\
    ...
    train_dataset=BigImageDataset(round(TrainInstances),shape_dset,
                            train=0,data_dir=data_dir, real=True)
    train_loader=data.DataLoader(train_dataset,batch_size=BatchSize,shuffle=True)
    #validation
    val_dataset=BigImageDataset(round(ValInstances),shape_dset,
                            train=1,data_dir=data_dir, train_size=TrainInstances, real=True)
    ...
    for learning_rate in lr_list:
        ...
        outputs_S = to_var(torch.zeros([1,1,m,n,p]))
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch % chk) ==0:
                torch.save(net.state_dict(), 
                    "/results/%s/%s_Res3d_Model_Tr%s_epoch%s_%s_lr%.2e.pkl"\
            ...
            if loss_val_mean<minloss:
                ...
                log.write('Best saved at [epoch%d/%d]\n'\
                    %(epoch+1,num_epochs))
                torch.save(net.state_dict(), 
                        "/results/%s/%s_Res3d_Best_Model_Tr%s_epoch%s_lr%.2e.pkl"\
            ...
            np.savez('/results/%s/%s_Res3d_LossData_Tr%s_epoch%s_lr%.2e'\
                %(ProjectName,prefix,TrainInstances,num_epochs,learning_rate),
        ...
        params={'ProjectName':ProjectName,
                ...
                'frame':frame}
        file=open('/results/%s/%s_Res3d_Settings_Tr%s_epoch%s_lr%.2e.txt'\
